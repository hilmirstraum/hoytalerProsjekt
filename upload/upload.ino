...
void loop(){
  ...
  if (newVolume < volume){
    ...
    if ((volume - newVolume)^0.5/2 < (millis())-volumeChangeStartTime){ //beregner om singalet er sendt lenge nok til at den er ferdig med endringne i volumet
      volume = newVolume;
  ...
  //volumet er under det ønskede volumet så det må justeres oppover
    ...
    if ((newVolume - volume)^0.5/2 < (millis())-volumeChangeStartTime){ //beregner om singalet er sendt lenge nok til at den er ferdig med endringne i volumet
      volume = newVolume;
      if (newVolume == 100){}
      digitalWrite(volumeUp, LOW);
